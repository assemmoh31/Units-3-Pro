...
import { ArrowLeft, Printer, Trash2, FileText, Globe, History } from 'lucide-react';
import AdUnit from './AdUnit';

interface GeographyLayoutProps {
  title: string;
  category: string;
...
export interface GeoHistoryItem {
  id: string;
  title: string;
  result: string;
  timestamp: string;
...
const GeographyLayout: React.FC<GeographyLayoutProps & { history: GeoHistoryItem[], onClear: () => void }> = ({
  title,
  category,
  icon,
  children,
    ...
    <div className="w-full max-w-7xl mx-auto pb-12 animate-fade-in px-4">
      ...
      <div className="mb-8 pt-4">
         <div className="flex items-center gap-2 text-sm text-slate-500 dark:text-slate-400 mb-2">
            <Link to="/geography" className="hover:text-blue-600 dark:hover:text-blue-400 transition-colors">Geography Hub</Link>
            <span>/</span>
            <span className="font-medium text-slate-700 dark:text-slate-300">{category}</span>
         </div>
         <div className="flex items-start justify-between">
            <div className="flex items-center gap-4">
               <Link to="/geography" className="p-3 rounded-xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-500 hover:text-blue-600 hover:border-blue-200 transition-all shadow-sm group">
               ...
               <div>
                  ...
                  <p className="text-slate-500 text-sm mt-1 flex items-center gap-2">
                     <Globe className="w-3 h-3" /> Navigation & Mapping Tool
                  </p>
               </div>
            </div>
         </div>
           ...
           <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden flex flex-col h-[calc(100vh-200px)] sticky top-24">
              <div className="p-4 bg-slate-50/50 dark:bg-slate-900/30 border-b border-slate-100 dark:border-slate-700 flex items-center justify-between backdrop-blur-sm">
                 <h3 className="font-bold text-slate-700 dark:text-slate-200 flex items-center gap-2 text-sm uppercase tracking-wide">
                    <History className="w-4 h-4 text-blue-500" /> Field Log
                 </h3>
              ...
              <div className="flex-grow overflow-y-auto p-2 space-y-2 scrollbar-thin scrollbar-thumb-slate-200 dark:scrollbar-thumb-slate-700" id="print-area">
                 {history.length === 0 ? (
                    ...
                    history.map((item) => (
                       <div key={item.id} className="bg-slate-50 dark:bg-slate-700/30 p-3 rounded-lg border border-slate-100 dark:border-slate-700/50 group hover:border-blue-200 dark:hover:border-blue-800 transition-colors">
                          <div className="flex justify-between items-start mb-2">
                             <span className="text-[10px] font-bold uppercase tracking-wider text-blue-600 bg-blue-50 dark:bg-blue-900/20 px-1.5 py-0.5 rounded">{item.title}</span>
