...
import { healthCategories, healthCalculators } from '../utils/health-data';
import { ArrowRight, HeartPulse } from 'lucide-react';
import SEO from '../components/SEO';

const HealthFitnessDashboard: React.FC = () => {
  return (
    <div className="w-full flex flex-col items-center gap-12 animate-fade-in mb-12">
      <SEO 
        title="Health & Fitness Calculators" 
        description="Calculate BMI, BMR, TDEE, Macros, and Body Fat percentage." 
        keywords={['health calculator', 'bmi calculator', 'calorie calculator', 'fitness tools']} 
      />
      {/* Hero Header */}
      <div className="text-center max-w-4xl px-4 mt-8 space-y-4">
        <div className="inline-flex items-center justify-center p-4 bg-rose-100 dark:bg-rose-900/30 rounded-full mb-4 text-rose-600 dark:text-rose-400 ring-4 ring-rose-50 dark:ring-rose-900/20">
           <HeartPulse className="w-8 h-8" />
        </div>
        <h1 className="text-4xl sm:text-6xl font-extrabold text-slate-900 dark:text-white tracking-tight">
          Health & <span className="text-transparent bg-clip-text bg-gradient-to-r from-rose-500 to-pink-600">Fitness</span>
        </h1>
        <p className="text-lg sm:text-xl text-slate-600 dark:text-slate-300 max-w-2xl mx-auto leading-relaxed">
          Optimize your wellbeing with professional calculators for metrics, nutrition, and training.
        </p>
      </div>

      <div className="w-full max-w-7xl px-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {healthCategories.map((cat: any) => {
           const tools = Object.values(healthCalculators).filter(c => c.category === cat.title);
                   ...
                   {tools.length > 0 ? (
                     ...
                     {tools.map(tool => (
                       ...
                       >
                         ...
                         <ArrowRight className="w-3 h-3 text-slate-300 group-hover/item:text-rose-500 transition-transform group-hover/item:translate-x-1" />
                       </Link>
                     ))}
                     </div>
                   ) : (
...
export default HealthFitnessDashboard;
