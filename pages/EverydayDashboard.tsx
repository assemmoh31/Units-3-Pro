...
import { everydayCategories, everydayCalculators } from '../utils/everyday-data';
import { ArrowRight, Coffee, Ruler } from 'lucide-react';
import SEO from '../components/SEO';

const EverydayDashboard: React.FC = () => {
  return (
    <div className="w-full flex flex-col items-center gap-12 animate-fade-in mb-12">
      <SEO 
        title="Everyday Unit Converters" 
        description="Quick tools for daily life: Temperature, Length, Weight, Volume, Speed, and Clothing sizes." 
        keywords={['unit converter', 'cooking converter', 'temperature', 'length', 'weight', 'shoe size']} 
      />
      {/* Hero Header */}
      <div className="text-center max-w-4xl px-4 mt-8 space-y-4">
        <div className="inline-flex items-center justify-center p-4 bg-orange-100 dark:bg-orange-900/30 rounded-full mb-4 text-orange-600 dark:text-orange-400 ring-4 ring-orange-50 dark:ring-orange-900/20">
           <Coffee className="w-8 h-8" />
        </div>
        <h1 className="text-4xl sm:text-6xl font-extrabold text-slate-900 dark:text-white tracking-tight">
          Everyday <span className="text-transparent bg-clip-text bg-gradient-to-r from-orange-500 to-yellow-500">Conversions</span>
        </h1>
        <p className="text-lg sm:text-xl text-slate-600 dark:text-slate-300 max-w-2xl mx-auto leading-relaxed">
          Quick and accurate tools for daily life. 
          Convert temperature, length, weight, volume, and more instantly.
        </p>
      </div>

      <div className="w-full max-w-7xl px-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {everydayCategories.map((cat: any) => {
           const tools = Object.values(everydayCalculators).filter(c => c.category === cat.title);
                   ...
                   {tools.length > 0 ? (
                     ...
                     {tools.map(tool => (
                       ...
                       >
                         ...
                         <ArrowRight className="w-3 h-3 text-slate-300 group-hover/item:text-orange-500 transition-transform group-hover/item:translate-x-1" />
                       </Link>
                     ))}
                     </div>
                   ) : (
                     <div className="p-6 text-center text-slate-400 text-xs italic flex flex-col items-center">
                        <Ruler className="w-6 h-6 mb-2 opacity-20" />
                        Coming Soon
                     </div>
                   )}
                </div>
